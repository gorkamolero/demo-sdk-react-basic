...
const RadioWithImages = ({field, title, onChangeHandler, size}) => {
  ...
  return (
    <>
      {meta.showTitle && <label style={{ marginBottom: ['small', 'medium'].includes(size)  ? 16 : 20, textAlign: 'center' }}>{title}</label>}
      <HbRadio
        selected={selected}
...
const CheckboxGroup = ({field, title, fieldValues, onChangeHandler, size}) => {
  ...
  return (
    <FlexBox column align="center" className="CheckboxGroup">
      {meta.showTitle && <label style={{ marginBottom: ['small', 'medium'].includes(size)  ? 16 : 20 }}>{title}</label>}
